...
vector<int> solve(int l, int r, int p) {
  ...
  int mid = (l + r) >> 1;
  vector<int> a = solve(l, mid, p), b = solve(mid+1, r, p);
  int len = a.size() + b.size() - 1;
  int z = 1;
  while ((1 << z) < len) ++z;
  vcd va(1 << z, 0), vb(1 << z, 0);
  ...
  a.resize(len);
  for (int i = 0; i < len; ++i) {
    long long now = (long long)(va[i].real() + 0.5);
    now %= p;
    if (now < 0) now += p;
    a[i] = now;
  }
...
int main() {
  ...
  while (t--) {
    ...
    cin >> b >> p;
    b = b+1;
    long long ans = calc(b % p, p) % mod;
    b /= p;
    while (b > 0) {
      ans = 1LL * ans * ((b % p) + 1) % mod;
      b /= p;
    }
    ans %= mod;
    if (ans < 0) ans += mod;
