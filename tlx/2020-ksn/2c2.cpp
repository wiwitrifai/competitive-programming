...
void answer(vector<int> s) {
  ...
  cerr << endl;
  cerr << " score " << score << "\t" << 1.0 / (1024.0 * score) << endl;
...
void subtask2(int n) {
  ...
  vector<int> res_first = ask(a);
  upd(res_first, a);
  check();

  vector<vector<int>> b;
  for (int bx = 0; bx * 2 < len; ++bx) {
    int bit = bx + len / 2;
    vector<int> s;
    for (int i = 0; i < n; ++i) {
      if (!active[i]) {
  ...
  vector<int> res_second = ask(b);
  upd(res_second, b);
  check();
  vector<int> val(len, -1);
  for (int i = 0; i < len/2; ++i) {
    if (res_first[i * 2] != res_first[i * 2 + 1]) {
      val[i] = res_first[i * 2];
    }
    if (res_second[i * 2] != res_second[i * 2 + 1]) {
      val[len/2 + i] = res_second[i * 2];
    }
  }
